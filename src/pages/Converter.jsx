...
useEffect(() => {
  ...
  const fromConvert = `1 ${fromCurrency} = ${(1 / ratesRef.current[fromCurrency] * ratesRef.current[toCurrency]).toFixed(2)}`
  const toConvert = `1 ${toCurrency} = ${(ratesRef.current[fromCurrency] / ratesRef.current[toCurrency]).toFixed(2)} ${fromCurrency}`
